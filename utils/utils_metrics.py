...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):
    ...
    for ind in range(len(gt_imgs)):
        ...
        # ------------------------------------------------#
        pred = np.array(cv2.imread(pred_imgs[ind], cv2.IMREAD_GRAYSCALE))
        label = np.array(cv2.imread(gt_imgs[ind], cv2.IMREAD_GRAYSCALE))
        # print(pred_imgs[ind])
        # print(gt_imgs[ind])
...
def compute_mIoU_gray(gt_dir, pred_dir, png_name_list, num_classes, name_classes, threshold=0.5):
    print('Num classes', num_classes)
    # -----------------------------------------#
    #   创建一个全是0的矩阵，是一个混淆矩阵
    # -----------------------------------------#
    ...
    # ------------------------------------------------#
    # gt_imgs = [join(gt_dir, x + "_manual1.png") for x in png_name_list]
    gt_imgs = [join(gt_dir, x + "_manual2.png") for x in png_name_list]
    ...
    for ind in range(len(gt_imgs)):
        ...
        pred = np.array([int(x) for x in pred.flatten()])
        pred = (pred > threshold * 255).astype(np.uint8)
        label = (label > threshold * 255).astype(np.uint8)
